...
function execute(key, page) {
    if (!page) page = 0;
    return customFetch(API_URL + "/manga?title=" + key + "&limit=20&offset=" + page + "&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&includes[]=cover_art&order[relevance]=desc")
        .then(async response => {
            if (response.ok) {
